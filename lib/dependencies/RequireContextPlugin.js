						...
						return [].concat(
							...items.map(obj => {
								...
								return this.extensions
									.filter(
										ext =>
											request.length > ext.length &&
											request.slice(-ext.length) === ext
									)
									.map(ext => ({
										context,
					...
					items => {
						...
						for (const obj of items) {
							...
							for (const mainFile of this.mainFiles) {
								if (
									request.length > mainFile.length + 1 &&
									request.slice(-mainFile.length - 1) === "/" + mainFile
								) {
									result.push({
										context,
										request: request.substr(0, -mainFile.length)
									});
									result.push({
										context,
										request: request.substr(0, -mainFile.length - 1)
									});
								}
							}
							result.push(obj);
